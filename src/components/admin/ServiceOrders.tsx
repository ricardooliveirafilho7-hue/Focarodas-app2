...
import { Search, Filter, Eye, Edit2, Calendar, Clock, Image as ImageIcon, Users, ClipboardList, Plus } from 'lucide-react';
...
export default function ServiceOrders() {
  ...
  const filtered = serviceOrders.filter(os => {
    ...
    if (filter === 'Pronto' && os.status !== 'Pronto' && os.status !== 'Finalizado' && os.status !== 'Retirada') return false;
    
    if (search) {
      const q = search.toLowerCase();
      const vehicle = getVehicleById(os.vehicleId);
      if (!(vehicle?.model || '').toLowerCase().includes(q) && !(vehicle?.plate || '').toLowerCase().includes(q) && !(os.id || '').toLowerCase().includes(q)) return false;
    }
    return true;
  });
    ...
    <div className="p-4 md:p-8">
      <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button className="h-[42px] px-4 rounded-xl border border-white/10 bg-[#1A1A1A] hover:bg-[#222] flex items-center gap-2 text-sm font-medium transition-colors shrink-0">
            ...
            <div key={os.id} className="bg-[#1A1A1A] border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-colors flex flex-col">
              <div className="flex justify-between items-start mb-4">
                 <div className="flex-1">
                   <p className="text-[10px] text-white/40 uppercase mb-1 font-mono tracking-wider">#{os.id}</p>
              ...
              <div className="flex justify-between items-end mt-5 pt-4 border-t border-white/5">
                <div>
                  ...
                  <p className="text-sm font-medium flex items-center gap-1.5 min-h-[20px]">
                    <Calendar size={14} className="text-white/40"/> {new Date(os.deliveryEstimate).toLocaleDateString('pt-BR')}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button 
                    onClick={() => setViewingOrder(os)}
                    className="w-8 h-8 rounded-lg border border-white/10 bg-[#222] hover:bg-white hover:text-black flex items-center justify-center transition-all text-white/60"
                  >
                    <Eye size={16} />
                  </button>
                  <button 
                    onClick={() => setEditingOrder(os)}
                    className="w-8 h-8 rounded-lg border border-white/10 bg-[#222] hover:bg-[var(--color-brand-red)] hover:border-[var(--color-brand-red)] flex items-center justify-center transition-all text-white/60 hover:text-white"
                  >
                    <Edit2 size={16} />
                  </button>
                </div>
      ...
      {filtered.length === 0 && (
         <div className="flex flex-col items-center justify-center py-20 text-center bg-[#1A1A1A]/50 border border-dashed border-white/10 rounded-3xl mt-4">
            ...
            <p className="text-white/40 mb-6 max-w-sm">Crie uma nova ordem de serviço para começar.</p>
            <button 
               onClick={() => setIsNewServiceModalOpen(true)}
      ...
      {editingOrder && (
        <EditServiceOrderModal order={editingOrder} onClose={() => setEditingOrder(null)} />
      )}
      
      {viewingOrder && (
        <ViewServiceOrderModal order={viewingOrder} onClose={() => setViewingOrder(null)} />
      )}
